...
def _redirect_info(request: Request, configured: str) -> dict:
    return google_hub.pick_redirect(str(request.base_url), configured)
...
<div class="panel">
  ...
  <div class="field"><label>Autorisierte Weiterleitungs-URI (bei Google eintragen)</label>
    <div class="x-form" style="align-items:center"><input type="text" id="g-redirect" readonly value="{esc(red["uri"])}" style="min-width:340px" onclick="this.select()">
    <button class="btn secondary sm g-copy" type="button" data-target="g-redirect">Kopieren</button></div>
    <div class="help">{esc(red["reason"])}</div></div>
  <form method="post" action="/admin/google/client">
    ...
    <details style="margin:8px 0"><summary class="x-muted">Erweitert: eigene Weiterleitungs-URI</summary>
      <div class="field" style="margin-top:8px"><input type="text" name="redirect_uri" value="{esc(summ["redirect_uri"])}" placeholder="leer = automatisch (siehe oben)">
      <div class="help">Nur nötig, wenn ASTRA unter einer eigenen https-Domain erreichbar ist.</div></div></details>
    <button class="btn sm" type="submit">Client speichern</button>
  </form>
</div>"""
...
def _err(msg: str) -> RedirectResponse:
    return RedirectResponse(f"/admin/google?err={quote(str(msg)[:400])}", status_code=303)
...
async def google_client_save(request: Request, _: bool = Depends(auth.require_admin)):
    ...
    await google_hub.set_client(cid, str(form.get("client_secret") or ""), str(form.get("redirect_uri") or ""))
    await db.audit("google_client_saved", actor="owner", detail={})
...
async def finish(code: str, state: str, error: str) -> RedirectResponse:
    ...
    if not code:
        return _err("In der Adresse fehlt der Code.")
    try:
        ...
        return _err(f"Anmeldung fehlgeschlagen: {e}")
    ...
    return RedirectResponse("/admin/google?saved=connected", status_code=303)
...
async def google_paste(request: Request, _: bool = Depends(auth.require_admin)):
    ...
    if parsed["error"]:                                        # Google hat die Anmeldung abgelehnt (z. B. access_denied)
        return await finish("", parsed["state"], parsed["error"])
    if not parsed["state"]:
        return _err("Füge die komplette Adresse ein (mit „state=“), nicht nur den Code.")
    return await finish(parsed["code"], parsed["state"], "")
